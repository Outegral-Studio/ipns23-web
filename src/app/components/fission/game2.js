...
const Game = (props) => {
	...
	const animate = useCallback(() => {
		...
		if(isBang.current) {
			...
			if(hole.current.r*hole.current.r > ctx.current.canvas.width*ctx.current.canvas.width/4 + ctx.current.canvas.height*ctx.current.canvas.height/4){
	...
	const startGame = useCallback(() => {
		...
		for(let i = 0; i < 60; i++) {
			...
			obstacles.current.push(new Obstacle(x, y, canvasHeight.current / 45, color,angle));
		}
